...
void Test3() {
    ...
    assert(Obj::GetAliveObjectCount() == 0);
    {
        ...
        assert(Obj::num_copied - num_copies == MEDIUM_SIZE -
            (MEDIUM_SIZE / 2));
    }
}
...
void Test4() {
...

    {
        ...
        // Операция PushBack существующего элемента вектора должна быть
        // безопасна даже при реаллокации памяти
        v.PushBack(v[0]);
        assert(v[0].IsAlive());
        assert(v[1].IsAlive());
    }
    {
        Vector<TestObj> v(1);
        assert(v.Size() == v.Capacity());
        // Операция PushBack для перемещения существующего элемента
        // вектора должна быть безопасна даже при реаллокации памяти
...
void Test5() {
    ...
    assert(Obj::GetAliveObjectCount() == 0);
    {
        Vector<TestObj> v(1);
        assert(v.Size() == v.Capacity());
        // Операция EmplaceBack существующего элемента вектора должна
...
void Test6() {
    ...
    }
    {
        ...
        Vector<Obj> v{ SIZE };
        v.Reserve(SIZE * 2);
        std::cout << "Obj::num_move_assigned = " << Obj::num_move_assigned << std::endl;
...
int main() {
    ...
    Vector<Obj> v(SIZE);
    Obj obj{ 1 };
    Vector<Obj>::iterator pos = v.Emplace(v.begin(), obj);
    std::cout << "num_copied = "s << Obj::num_copied << std::endl;
    ...
    std::cout << "v1.size = " << v1.Size() << " v1.capacity = " << v1.Capacity()
        << " v2.size = " << v2.Size() << " v2.capacity = " << v2.Capacity() << std::endl;
